...
export const ExpiryReport = () => {
  ...
  return (
    <div className="p-4 md:p-8 space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out font-sans">
      ...
      <Card className="border-slate-200/60 dark:border-slate-800 shadow-[0_8px_30px_rgb(0,0,0,0.04)] bg-white/50 dark:bg-slate-950/50 backdrop-blur-sm rounded-[24px] md:rounded-[28px] overflow-hidden">
        <Tabs defaultValue="today" className="w-full">
          ...
          <div className="px-4 md:px-6 pt-6 pb-2 border-b border-slate-100 dark:border-slate-800 bg-slate-50/30 dark:bg-slate-900/20">
            {/* Added overflow-x-auto so tabs don't shrink too small on phones */}
            <div className="overflow-x-auto pb-2 scrollbar-hide">
              <TabsList className="h-11 md:h-12 flex w-max min-w-full md:w-full md:max-w-2xl bg-slate-100 dark:bg-slate-900 p-1 rounded-xl border border-slate-200/50 dark:border-slate-800">
                {tabs.map(t => (
                  <TabsTrigger 
                    key={t.key} 
                    value={t.key}
                    className={cn(
                      "flex-1 px-4 gap-2 text-[11px] md:text-[12px] font-bold tracking-tight transition-all duration-300 rounded-lg whitespace-nowrap",
                  ...
                  >
                    ...
                    {t.label} {/* Restored text */}
                    <span className="ml-0.5 px-2 py-0.5 rounded-md bg-slate-200 dark:bg-slate-700 text-[10px] font-mono font-bold opacity-80">
                      {t.data.length}
                    </span>
                  </TabsTrigger>
                ))}
      ...
      {/* Syncing Indicator */}
      {loading && (
        <div className="fixed bottom-6 right-6 md:bottom-8 md:right-8 animate-pulse bg-slate-900 dark:bg-white text-white dark:text-slate-900 text-[9px] font-bold tracking-[0.2em] px-5 py-2.5 rounded-full shadow-2xl backdrop-blur-md z-50">
          SYNCING LIVE DATA
        </div>
      )}
    </div>
